...
class ArgoDataset(Dataset):
    ...
    def get_obj_feats(self, data):
        ...
        # print(gt_preds.shape)
        # print(has_preds.shape)
        return data
    ...
    def get_lane_graph(self, data):
        ...
        for i, lane_id in enumerate(lane_ids):
            ...
            left['u'] += idcs[1:]
            left['v'] += idcs[:-1]
            if lane.l_neighbor_id is not None:
                nbr_id = lane.l_neighbor_id
                if nbr_id in lane_ids:
                    j = lane_ids.index(nbr_id)
                    left['u'].append(idcs[0])
                    left['v'].append(node_idcs[j][-1])
                    
            right['u'] += idcs[:-1]
            right['v'] += idcs[1:]
            if lane.r_neighbor_id is not None:
                nbr_id = lane.r_neighbor_id
                if nbr_id in lane_ids:
                    j = lane_ids.index(nbr_id)
                    right['u'].append(idcs[-1])
                    right['v'].append(node_idcs[j][0])
        ...
        for k1 in ['left', 'right']:
            for k2 in ['u', 'v']:
                graph[k1][k2] = np.asarray(graph[k1][k2], np.int64)
