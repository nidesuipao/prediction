...
def main():
    ...
    for ii, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            output = net(data)
            # print(output['reg'][0].shape)
            # print(data["gt_preds"][0][0].shape)
            results_ = [x[:].detach().cpu().numpy() for x in output["reg"][0]]
            results = [x[0:1].detach().cpu().numpy() for x in output["reg"]]

        # print(len(results))
        # print(data["gt_preds"][0].shape)

        for i, pred_traj in enumerate(results_):
            preds_[i] = pred_traj.squeeze()
            gts_[i] = data["gt_preds"][0][i] if "gt_preds" in data else None
        
        visual = True
        if visual is True:
            show_predict_result(data, preds_, gts_, "./save_img/val/"+str(ii)+".png" , True, None)
        ...
        for i, (argo_idx, pred_traj) in enumerate(zip(data["argo_id"], results)):
            ...
            # print(pred_traj.squeeze().shape)
            # print(data["gt_preds"][i][0].shape )
