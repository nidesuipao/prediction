...
def loss_(reg, gt_preds, num_mods, num_preds):
...
def PredLoss(out, y, max_n_guesses, horizon):

    out = out.view(-1, max_n_guesses, horizon * 2)
    y = y.view(-1, horizon * 2)
    print(out.shape)
    print(y.shape)
    dist = []
    ...
    min_dist, min_idcs = dist.min(1)
    row_idcs = torch.arange(len(min_idcs)).long().to(min_idcs.device)
    out = out[row_idcs, min_idcs]
    loss_out = reg_loss(out, y)
    return loss_out
