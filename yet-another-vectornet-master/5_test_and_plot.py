...
global_step = 0
FILENAME = '2023428/epoch_999.valminade_1.057.pth'#'epoch_1.valminade_14.072.20230425_2.pth'
checkpoint_dir = os.path.join('trained_params', FILENAME)
# checkpoint_dir = None
#NORM_CENTERS_DICT_DIR = os.path.join('interm_data', 'train-norm_center_dict.pkl')
...
if __name__ == "__main__":
    ...
    model = HGNN(in_channels, max_n_guesses * out_channels,3,1,64,64,64)
    ...
    for data in test_loader:
        dat = dict()
        dat["x"] = data.x.numpy().tolist()
        dat["edge_index"] = data.edge_index.numpy().tolist()
        dat["y"] = data.y.numpy().tolist()
        dat["cluster"] = data.cluster.numpy().tolist()
        dat["valid_len"] = data.valid_len.numpy().tolist()
        dat["time_step_len"] = data.time_step_len.numpy().tolist()
        dat["batch"] = data.batch.numpy().tolist()
        dat["ptr"] = data.ptr.numpy().tolist()
        b = json.dumps(dat)
        ...
        out = model(data.x, 
                    ...
                    data.batch,
                    data.ptr)
        #out = out.view(max_n_guesses, -1, out_channels)
        #data = data.to(device)
        ...
        test_data_path = test_data.file_names[index][:-3]
        test_data_path = os.path.join("./interm_data/val_intermediate", test_data_path)
        data_ = pd.read_pickle(test_data_path)
        #print(data_)
        add_len = data_['TARJ_LEN'].values[0]
        # print("ssssssssss",add_len)
        
        show_predict_result(data_, out, gt, add_len, show_lane= True, 
                            save_path= "./save_img/val/" + str(index) +".png", 
                            loss = str(round(loss.item(), 3)))
...
#%%
    ...
    # metrics = get_eval_metric_results(model, test_loader, device, out_channels, max_n_guesses, horizon, miss_threshold)
    # curr_minade = metrics["minADE"]
    # if curr_minade < best_minade:
    #     best_minade = curr_minade
