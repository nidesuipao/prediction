...
def show_predict_result(data, pred_y: torch.Tensor, y, add_len, save_path, show_lane=True, loss = None):
    ...
    pred_y = pred_y.detach().numpy().reshape((-1, 2)).cumsum(axis=0)
    y = y.detach().numpy().reshape((-1, 2)).cumsum(axis=0)
